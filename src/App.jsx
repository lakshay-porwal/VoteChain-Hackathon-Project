...
const CONTRACT_ADDRESS = "0x64c2a4dF41c01d6Dc59A9a29c7d777d37Dc09d9f";
...
const Web3Provider = ({ children }) => {
  ...
  const addNotification = useCallback((message, type = 'info', targetGroup = 0) => {
    const id = Date.now();
    const newNotif = { id, message, type, timestamp: id, read: false, targetGroup };
  ...
  const connectWallet = async () => {
    ...
    try {
      ...
      if (contractInstance) {
        ...
        try {
          const superAdminAddr = await contractInstance.superAdmin();
          const isUserAdmin = await contractInstance.isAdmin(acc);
          
          if (superAdminAddr.toLowerCase() === acc.toLowerCase()) {
            setIsSuperAdmin(true);
            setIsAdmin(true);
            addNotification("Welcome Super Admin", "success");
          } else if (isUserAdmin) {
            setIsAdmin(true);
          ...
          addNotification("Connected, but could not verify Admin status.", "warning");
        }
      }
    } catch (error) {
      console.error(error);
  ...
  return (
    <Web3Context.Provider value={{
      account, isAdmin, isSuperAdmin, isConnected, contract,
