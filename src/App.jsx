    ...
    try {
      ...
      try {
        ...
        if (err.response && err.response.status === 404) {
          // User not found – new user, needs to register
          ...
          addNotification("⚠️ Backend server is offline. Face auth unavailable.", "warning");
        }
      }

      // Check Admin Status on the smart contract
      const contractInstance = await getContract(signer);

      if (contractInstance) {
        setContract(contractInstance);
        try {
          // isAdmin is a public mapping: returns bool, should never revert for a valid address.
          // If it does revert, the contract address is wrong – treat as not-admin.
          const isAdminUser = await contractInstance.isAdmin(acc);
          if (isAdminUser) {
            setIsAdmin(true);
            addNotification("Welcome, Admin! 🛡️", "success");
          } else {
            addNotification("Wallet connected successfully! ✅", "success");
          }
        } catch (e) {
          // Contract call reverted – likely wrong contract address or ABI mismatch
          console.warn("isAdmin check failed (contract mismatch?):", e.shortMessage || e.message);
          // Do NOT show an error toast for this – treat as a regular voter
          addNotification("Wallet connected successfully! ✅", "success");
        }
      }
    } catch (error) {
...
const BackendWarningBanner = () => {
  ...
  return (
    <div className="fixed top-16 left-0 right-0 z-40 bg-orange-500/90 text-white text-sm text-center py-2 px-4 flex items-center justify-center gap-2 backdrop-blur-sm">
      <AlertCircle className="w-4 h-4 flex-shrink-0" />
      <span>Backend server is offline. Please start the server in <code className="bg-black/30 px-1 rounded">/server</code> to enable face authentication.</span>
    </div>
  );
};
...
const MainContent = () => {
  ...
  // If connected but not verified: show face auth (only if backend is online)
  if (isConnected && !isFaceVerified) {
    if (!backendOnline) {
      // Backend is offline – show a message instead of the face auth screen
      return (
        <div className="pt-16 min-h-screen bg-[#0f172a] text-white flex items-center justify-center pb-20">
          <div className="text-center p-8 bg-orange-500/10 border border-orange-500/30 rounded-2xl max-w-md">
            <AlertCircle className="w-16 h-16 text-orange-400 mx-auto mb-4" />
            <h2 className="text-2xl font-bold mb-2 text-orange-300">Backend Server Offline</h2>
            <p className="text-gray-400 text-sm leading-relaxed">
              The face authentication server is not running. Please start it by opening a new terminal and running:
            </p>
            <code className="block mt-4 p-3 bg-black/40 rounded-lg text-green-400 text-sm font-mono text-left">
              cd server<br />
              npm install<br />
              node index.js
            </code>
            <p className="text-gray-500 text-xs mt-4">Make sure MongoDB is running first.</p>
          </div>
        </div>
      );
    }
